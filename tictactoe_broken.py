...
def add_piece(game, player, row, column):
    ...
    """
    game[row][column] = player #Angeline removed +1 in column index
    return game
...
def switch_player(player):
    if player == 1: #Angeline changed = to ==
        return 2
    else:
        return 1
...
if __name__ == '__main__':
    ...
    while winner == 0 and moves_exist(game):
        ...
        available = False
        while not available: #Angeline added colon in while loop
            row = convert_input_to_coordinate(int(input("Which row? (start with 1) ")))
            column = convert_input_to_coordinate(int(input("Which column? (start with 1) ")))
            available = check_space_empty(game,row,column) #Angeline added column argument
